...
import {
  ...
  RotateCw,
  Zap,
  InspectionPanel as Intersection,
  MapPin,
  Frame as Wireframe,
  Cuboid as Cube,
  Ruler,
...
const Toolbar: React.FC<ToolbarProps> = ({ onOpenCatalog }) => {
  ...
  const [polylineMenuPosition, setPolylineMenuPosition] = useState({
    x: 0,
    y: 0,
  });
  ...
  const hasIntersectingShapes = React.useMemo(() => {
    ...
    try {
      ...
      const sMin = selectedBox.min
        .clone()
        .add(new THREE.Vector3(...selectedShape.position));
      const sMax = selectedBox.max
        .clone()
        .add(new THREE.Vector3(...selectedShape.position));
      ...
      return shapes.some((s) => {
        ...
        try {
          ...
          b.set(
            b.min.clone().add(new THREE.Vector3(...s.position)),
            b.max.clone().add(new THREE.Vector3(...s.position))
          );
  ...
  const getViewModeIcon = () => {
    ...
    if (viewMode === ViewMode.WIREFRAME)
      return <Wireframe size={14} className={cls} />;
  ...
  const handleSubtract = async () => {
    ...
    try {
      ...
      const selBox = new THREE.Box3().setFromBufferAttribute(
        sel.geometry.getAttribute('position')
      );
      selBox.set(
        selBox.min.clone().add(new THREE.Vector3(...sel.position)),
        selBox.max.clone().add(new THREE.Vector3(...sel.position))
      );
      ...
      for (const target of intersecting) {
        ...
        const relOff = sel.position.map((v, i) => v - target.position[i]) as [
          number,
          number,
          number
        ];
        const relRot = sel.rotation.map((v, i) => v - target.rotation[i]) as [
          number,
          number,
          number
        ];
        const result = await performBooleanCut(
          target.replicadShape,
          sel.replicadShape,
          undefined,
          relOff,
          undefined,
  ...
  const transformTools = [
    ...
    { id: Tool.MOVE, icon: <Move size={13} />, label: 'Move', shortcut: 'M' },
    {
      id: Tool.POINT_TO_POINT_MOVE,
  ...
  return (
    <div className="flex flex-col font-inter">
      ...
      <div className="flex items-center h-12 px-4 bg-stone-50 border-b border-stone-200 shadow-sm">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <button
            ...
            title={`Current: ${getViewModeLabel()} View`}
          >
            {getViewModeIcon()}
          ...
          <button
            ...
            title={`Linear Mode: ${orthoMode === OrthoMode.ON ? 'ON' : 'OFF'}`}
          >
            <Grid
              size={12}
              className={
                orthoMode === OrthoMode.ON ? 'text-white' : 'text-slate-800'
              }
            />
          ...
          <div className="relative">
            <Search
              size={14}
              className="absolute left-3 top-2 text-stone-500"
            />
            <input
              type="text"
              placeholder="Search..."
        ...
        <div className="flex items-center h-full">
          {menus.map((menu) => (
            <div key={menu.label} className="relative h-full">
              ...
              {activeMenu === menu.label && (
                <div
                  className="absolute left-0 top-full mt-1 w-52 bg-white rounded-lg border border-stone-200 py-1 z-50 shadow-xl"
                  ...
                  {menu.items.map((item, i) =>
                    item.type === 'separator' ? (
                      ...
                      <button
                        ...
                        onClick={() => {
                          if (item.label === 'Solid View')
                            setViewMode(ViewMode.SOLID);
                          else if (item.label === 'Wireframe View')
                            setViewMode(ViewMode.WIREFRAME);
                          else if (item.label === 'X-Ray View')
                            setViewMode(ViewMode.XRAY);
                          setActiveMenu(null);
                        }}
                      >
      ...
      {/* Toolbar */}
      <div className="flex items-center h-10 gap-1 px-4 bg-stone-50 border-b border-stone-200">
        <div className="flex items-center bg-white rounded-lg shadow-sm border border-stone-200 overflow-hidden">
          {[
            { icon: <FilePlus size={13} />, label: 'New', shortcut: 'Ctrl+N' },
            { icon: <Save size={13} />, label: 'Save', shortcut: 'Ctrl+S' },
            {
            ...
            <button
              key={i}
              className="p-1.5 text-stone-600 hover:bg-stone-100 hover:text-slate-800 transition-colors"
          ...
          {[
            { icon: <Undo2 size={13} />, label: 'Undo', shortcut: 'Ctrl+Z' },
            { icon: <Redo2 size={13} />, label: 'Redo', shortcut: 'Ctrl+Y' },
          ].map((btn, i) => (
            <button
              key={i}
              className="p-1.5 text-stone-600 hover:bg-stone-100 hover:text-slate-800 transition-colors"
        ...
        <div className="flex items-center bg-white rounded-lg shadow-sm border border-stone-200 overflow-hidden">
          {transformTools.map((tool) => {
            const isDisabled =
              ...
              (tool.disabledForBox && isBoxSelected);
            return (
              <button
                key={tool.id}
                className={`p-1.5 transition-all ${
                  activeTool === tool.id
                    ...
                    : 'hover:bg-stone-100 text-stone-600 hover:text-slate-800'
                }`}
                onClick={() =>
                  tool.id === Tool.SELECT
                    ? setActiveTool(tool.id)
                    : !isDisabled && handleTransformToolSelect(tool.id)
                }
                disabled={isDisabled}
                title={
        ...
        <div className="flex items-center bg-white rounded-lg shadow-sm border border-stone-200 overflow-hidden">
          <button
            ...
            className="p-1.5 transition-all hover:bg-stone-100 text-stone-600 hover:text-slate-800"
            title="Add Box (B)"
          >
            <Box size={13} />
          </button>
          ...
          <button
            onClick={() =>
              selectedShapeId && setShowParametersPanel(!showParametersPanel)
            }
            className={`p-1.5 transition-all ${
              selectedShapeId
                ? 'hover:bg-stone-100 text-stone-600 hover:text-slate-800'
      ...
      {showPolylineMenu && (
        <div
          className="fixed bg-white rounded-lg border border-stone-200 py-1 z-50 shadow-xl"
