      ...
      {selectedShape && selectedPanelRow !== null && (() => {
        ...
        return (
          <div className="border-t border-orange-200 bg-orange-50 px-3 py-2 rounded-b-lg">
            ...
            {(() => {
              const panel = currentPanelId ? shapes.find(s => s.id === currentPanelId) : null;
              const ops = panel?.faceExtrudeOperations || [];
              if (ops.length === 0) return null;
              return (
                <div className="mt-1.5 border-t border-orange-200 pt-1.5 space-y-1">
                  <div className="text-[9px] text-stone-400 font-semibold uppercase tracking-wider">Extrude Ops</div>
                  {ops.map((op, idx) => (
                    <div key={op.id} className="flex items-center gap-1.5 group">
                      <span className="text-[10px] font-mono font-bold text-slate-600 w-5 text-center">{op.axisLabel}</span>
                      <input
                        type="text"
                        inputMode="numeric"
                        ...
                        onBlur={async (e) => {
                          ...
                          if (isNaN(newVal) || newVal === op.value) return;
                          const updatedOps = ops.map((o, i) => i === idx ? { ...o, value: newVal } : o);
                          updateShape(panel!.id, { faceExtrudeOperations: updatedOps });
                          const latestPanel = useAppStore.getState().shapes.find(s => s.id === panel!.id);
                          if (!latestPanel) return;
                          const baseShape = latestPanel.parameters?.originalReplicadShape || latestPanel.replicadShape;
                          if (!baseShape || !latestPanel.geometry) return;
                          ...
                          const result = await applyAllFaceExtrudeOperations(baseShape, updatedOps, latestPanel.geometry);
                          if (!result) return;
                          ...
                          updateShape(panel!.id, {
                            ...
                            faceExtrudeOperations: updatedOps,
                            parameters: { ...latestPanel.parameters, width: sz.x, height: sz.y, depth: sz.z },
                          });
                        }}
                        onKeyDown={(e) => { if (e.key === 'Enter') (e.target as HTMLInputElement).blur(); }}
                        className="w-14 h-5 px-1 text-[11px] font-mono text-center border border-orange-200 rounded bg-white focus:outline-none focus:border-orange-500"
                      />
                      <span className="text-[9px] text-stone-400">{op.isFixed ? 'Fix' : 'Din'}</span>
                      <button
                        onClick={async () => {
                          const updatedOps = ops.filter((_, i) => i !== idx);
                          updateShape(panel!.id, { faceExtrudeOperations: updatedOps });
                          const latestPanel = useAppStore.getState().shapes.find(s => s.id === panel!.id);
                          if (!latestPanel) return;
                          const baseShape = latestPanel.parameters?.originalReplicadShape || latestPanel.replicadShape;
                          ...
                          if (updatedOps.length === 0) {
                            const { convertReplicadToThreeGeometry } = await import('./ReplicadService');
                            const geo = convertReplicadToThreeGeometry(baseShape);
