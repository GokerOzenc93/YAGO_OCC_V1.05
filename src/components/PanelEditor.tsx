...
const findPanel = (shapes: any[], pid: string, fi: number) => shapes.find(s => s.type === 'panel' && s.parameters?.parentShapeId === pid && s.parameters?.faceIndex === fi && !s.parameters?.extraRowId);
...
export function PanelEditor({ isOpen, onClose, embedded = false }: PanelEditorProps) {
  ...
  const activePanelId = useMemo(() => {
    ...
    if (typeof selectedPanelRow === 'number') return findPanel(shapes, selectedShape.id, selectedPanelRow)?.id || null;
  ...
  const createPanelForFace = async (fg: CoplanarFaceGroup, faces: FaceData[], fi: number) => {
    if (!selectedShape?.replicadShape) return;
    try {
      const verts: THREE.Vector3[] = []; fg.faceIndices.forEach(idx => faces[idx].vertices.forEach(v => verts.push(v.clone())));
      const n = fg.normal.clone().normalize(), center = new THREE.Vector3(); new THREE.Box3().setFromPoints(verts).getCenter(center);
      const { createPanelFromFace, convertReplicadToThreeGeometry } = await import('./ReplicadService');
      const rp = await createPanelFromFace(selectedShape.replicadShape, [n.x,n.y,n.z], [center.x,center.y,center.z], PANEL_THICKNESS, null); if (!rp) return;
      addShape(makePanelBase(selectedShape, { geometry: convertReplicadToThreeGeometry(rp), replicadShape: rp,
        parameters: { width: 0, height: 0, depth: PANEL_THICKNESS, parentShapeId: selectedShape.id, faceIndex: fi, faceRole: selectedShape.faceRoles?.[fi] } }));
    } catch (e) { console.error('Failed to create panel:', e); }
  };
  ...
  const panelContent = selectedShape ? (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        ...
        {loading ? <div className="px-2 py-0.5 text-xs text-stone-400 bg-white border border-gray-300 rounded" style={{ width: '30mm' }}>Loading...</div>
      ...
      {(() => {
        ...
        const faceRoles = selectedShape.faceRoles || {}, faceDescs = selectedShape.faceDescriptions || {}, facePanels = selectedShape.facePanels || {};
        ...
        const handleToggle = async (fi: number) => {
          if (isOff) return; const fp = { ...facePanels };
          if (fp[fi]) { delete fp[fi]; const p = findPanel(shapes, sid, fi); if (p) useAppStore.getState().deleteShape(p.id); }
          else { fp[fi] = true; await createPanelForFace(faceGroups[fi], faces, fi); }
          updateShape(sid, { facePanels: fp, faceGroupDescriptors: buildDesc() }); await resolveIfActive(sid);
        };
        ...
        const createVP = async (_: string, vi: number) => {
          ...
          try {
            ...
            addShape(makePanelBase(selectedShape, { geometry: g, replicadShape: rp,
              parameters: { width: s[def], height: s[alt], depth: PANEL_THICKNESS, parentShapeId: sid, faceIndex: -(vi+1), faceRole: vf.role, virtualFaceId: vf.id, arrowRotated: false } }));
            updateVirtualFace(vf.id, { hasPanel: true });
        ...
        const roleSel = (val: string, onChange: (v: string) => void, bc = 'border-gray-300') => (
          <select value={val} disabled={isOff} onClick={stop} onChange={e => onChange(e.target.value)} style={{ width: '32mm' }}
            className={`px-1 py-0.5 text-xs border rounded ${isOff ? 'bg-stone-100 text-stone-400 border-stone-200' : `bg-white text-gray-800 ${bc}`}`}>
            <option value="">none</option>{ROLE_OPTIONS.map(r => <option key={r} value={r}>{r}</option>)}</select>
        );
        return (
          <div className={`space-y-0.5 pt-2 border-t border-stone-300 ${isOff ? 'opacity-40 pointer-events-none' : ''}`}>
            {resolving && <div className="text-[10px] font-normal text-orange-500 animate-pulse mb-1">resolving joints...</div>}
            {order.map(i => {
              const sel = selectedPanelRow === i, fl = fgl.get(i), lt = fl?.label ?? `${i+1}`, lc = fl?.color ?? '#1a1a1a', hp = !!facePanels[i];
              return (
                <div key={`f-${i}`} ref={el => { if (el) rowRefs.current.set(i, el); else rowRefs.current.delete(i); }}
                  className={`flex w-fit gap-0.5 items-center rounded transition-colors ${sel ? 'bg-red-100 ring-1 ring-red-600' : 'hover:bg-gray-50'} ${hp ? 'cursor-pointer' : ''}`}
                  onClick={e => { stop(e); if (hp) setSelectedPanelRow(i, null, sid); }}>
                  <input type="radio" name="ps" checked={sel} disabled={isOff||!hp} onChange={e => { stop(e); setSelectedPanelRow(i,null,sid); }}
                    className={`w-4 h-4 ${isOff||!hp ? 'text-stone-300 cursor-not-allowed' : 'text-orange-600 focus:ring-orange-500 cursor-pointer'}`} onClick={stop} />
                  <input type="text" value={lt} readOnly tabIndex={-1} disabled={isOff} style={isOff ? undefined : { color: lc }}
                    className={`w-10 px-1 py-0.5 text-xs font-mono font-bold border rounded text-center ${isOff ? 'bg-stone-100 text-stone-400 border-stone-200' : 'bg-white border-gray-300'}`} onClick={stop} />
                  {roleSel(faceRoles[i]||'', async v => {
                    ...
                    const ps = findPanel(shapes, sid, i);
                    if (ps) { updateShape(ps.id, { parameters: { ...ps.parameters, faceRole: nr } }); await rebuildIfActive(sid); }
                  })}
                  <input type="text" value={faceDescs[i]||''} disabled={isOff} onClick={stop}
                    ...
                    className={`px-2 py-0.5 text-xs border rounded ${isOff ? 'bg-stone-100 text-stone-400 border-stone-200 placeholder:text-stone-300' : 'bg-white text-gray-800 border-gray-300'}`} />
                  <div className="ml-3 flex items-center gap-0.5">
                    <input type="checkbox" checked={hp} disabled={isOff} onClick={stop} onChange={() => handleToggle(i)}
                      className={`w-4 h-4 border-gray-300 rounded ${isOff ? 'text-stone-300 cursor-not-allowed' : 'text-green-600 focus:ring-green-500 cursor-pointer'}`} title={`Toggle panel for face ${lt}`} />
                    <button disabled={isOff||!hp} onClick={e => { stop(e); toggleArrow(findPanel(shapes, sid, i)); }}
                      className={`p-0.5 rounded transition-colors ${isOff||!hp ? 'text-stone-300 cursor-not-allowed' : findPanel(shapes,sid,i)?.parameters?.arrowRotated ? 'text-blue-600 bg-blue-50 hover:bg-blue-100' : 'text-slate-500 hover:bg-stone-100'}`}
                      title="Rotate arrow direction"><RotateCw size={13} /></button>
                  </div>
                </div>);
            })}
            {svf.map((vf, vi) => {
              ...
              return (
                <div key={vf.id} className={`flex w-fit gap-0.5 items-center rounded transition-colors ${sel ? 'bg-red-100 ring-1 ring-red-600' : 'hover:bg-gray-50'} ${vf.hasPanel ? 'cursor-pointer' : ''}`}
                  onClick={e => { stop(e); rc(); }}>
                  <input type="radio" name="ps" checked={sel} disabled={isOff||!vf.hasPanel} onChange={e => { stop(e); rc(); }}
                    className={`w-4 h-4 ${isOff||!vf.hasPanel ? 'text-stone-300 cursor-not-allowed' : 'text-orange-600 focus:ring-orange-500 cursor-pointer'}`} onClick={stop} />
                  <input type="text" value={`V${vi+1}`} readOnly tabIndex={-1} className="w-10 px-1 py-0.5 text-xs font-mono border rounded text-center bg-green-100 text-green-800 border-green-300" onClick={stop} />
                  {roleSel(vf.role||'', async v => {
                    const nr = v===''?null:v as FaceRole; updateVirtualFace(vf.id, { role: nr });
                    if (vp) { updateShape(vp.id, { parameters: { ...vp.parameters, faceRole: nr } }); await rebuildIfActive(sid); }
                  }, 'border-green-300')}
                  <input type="text" value={vf.description||''} disabled={isOff} onClick={stop} onChange={e => updateVirtualFace(vf.id, { description: e.target.value })}
                    placeholder="description" style={{ width: '40mm' }}
                    className={`px-2 py-0.5 text-xs border rounded ${isOff ? 'bg-stone-100 text-stone-400 border-stone-200 placeholder:text-stone-300' : 'bg-white text-gray-800 border-green-300'}`} />
                  <div className="ml-3 flex items-center gap-0.5">
                    <input type="checkbox" checked={vf.hasPanel} disabled={isOff} onClick={stop}
                      ...
                      className={`w-4 h-4 border-gray-300 rounded ${isOff ? 'text-stone-300 cursor-not-allowed' : 'text-green-600 focus:ring-green-500 cursor-pointer'}`} title={`Toggle virtual face V${vi+1}`} />
                    <button disabled={isOff||!vf.hasPanel} onClick={e => { stop(e); toggleArrow(vp); }}
                      className={`p-0.5 rounded transition-colors ${isOff||!vf.hasPanel ? 'text-stone-300 cursor-not-allowed' : ar ? 'text-blue-600 bg-blue-50 hover:bg-blue-100' : 'text-slate-500 hover:bg-stone-100'}`}
                      title="Rotate arrow direction"><RotateCw size={13} /></button>
                    <button disabled={isOff} onClick={e => { stop(e); if (vf.hasPanel) removeVP(vf.id); deleteVirtualFace(vf.id); }}
                      className="p-0.5 hover:bg-red-100 rounded transition-colors" title="Delete virtual face"><Trash2 size={13} className="text-red-400" /></button>
                  </div>
                </div>);
            })}
          </div>);
  ...
  const dimsSection = selectedShape && selectedPanelRow !== null && (() => {
    ...
    if (typeof selectedPanelRow === 'number') {
      const ps = findPanel(shapes, selectedShape.id, selectedPanelRow); cpId = ps?.id||null;
      if (ps?.geometry) dims = getDimsFromGeo(ps.geometry, ps.parameters?.faceRole, ps.parameters?.arrowRotated);
  ...
  if (embedded) return (<div className="flex flex-col"><div className="px-3 py-2 border-b border-stone-200 bg-stone-50">{panelToolbar}</div><div className="p-3">{panelContent}</div>{dimsSection}</div>);
  ...
  return (
    <div className="fixed bg-white rounded-lg shadow-2xl border border-stone-300 z-50" style={{ left: `${position.x}px`, top: `${position.y}px`, width: '410px' }}>
      <div className="flex items-center justify-between px-3 py-2 bg-stone-100 border-b border-stone-300 rounded-t-lg select-none" style={{ cursor: isDragging ? 'grabbing' : 'grab' }} onMouseDown={handleMouseDown}>
        <div className="flex items-center gap-2"><GripVertical size={14} className="text-stone-400"/><span className="text-sm font-semibold text-slate-800">Panel Editor</span></div>
        <div className="flex items-center gap-1">{panelToolbar}<button onClick={onClose} className="p-0.5 hover:bg-stone-200 rounded transition-colors"><X size={14} className="text-stone-600"/></button></div>
      </div>
      <div className="p-3 max-h-[calc(100vh-200px)] overflow-y-auto">{panelContent}</div>
      {dimsSection}
    </div>);
}
