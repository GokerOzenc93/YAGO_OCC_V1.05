...
function makeFaceTex(label: string, hovered: boolean, flipH = false): THREE.CanvasTexture {
  ...
  while (ctx.measureText(label).width > maxW && fs > 14) { fs -= 2; ctx.font = `800 ${fs}px "Syne","DM Sans",system-ui,sans-serif`; }
...
const FacePlane: React.FC<{ geo: THREE.BufferGeometry; pos:[number,number,number]; rot:[number,number,number]; label?:string; isEdge?:boolean; flipH?:boolean; onSnap:()=>void }> =
({ geo, pos, rot, label='', isEdge=false, flipH=false, onSnap }) => {
  const [hov, setHov] = useState(false);
  const tex = useMemo(() => isEdge ? makeEdgeTex(hov) : makeFaceTex(label, hov, flipH), [hov, label, isEdge, flipH]);
...
const CornerZone: React.FC<{pos:[number,number,number]; onSnap:()=>void}> = ({pos, onSnap}) => {
  ...
  return (
    <mesh geometry={geo} position={pos} renderOrder={6}
      ...
      onPointerLeave={e=>{e.stopPropagation();setHov(false);document.body.style.cursor='default';}}
      onClick={e=>{e.stopPropagation();onSnap();}}>
      <meshBasicMaterial color={C_ACCENT} transparent opacity={hov?0.82:0} depthTest={false} depthWrite={false}/>
    </mesh>
  );
};

/* ── Compass ring — stays flat, rotates only around Y ── */
const CompassRing: React.FC = () => {
  ...
  /* Ring — slightly thicker (inner/outer radii closer) */
  const ringGeo = useMemo(() => new THREE.RingGeometry(0.68, 0.78, 64), []);
  const ringMat = useMemo(() => new THREE.MeshBasicMaterial({
    color: '#9c9690', side: THREE.DoubleSide, transparent: true, opacity: 0.70, depthTest: false,
  }), []);

  /* Cardinal tick lines — 4 thick ticks at N/E/S/W */
  const tickLines = useMemo(() => {
    return [0, Math.PI / 2, Math.PI, -Math.PI / 2].map(angle => {
      const inner = 0.78, outer = 0.96;
      const pts = [
        new THREE.Vector3(Math.sin(angle) * inner, Math.cos(angle) * inner, 0),
        new THREE.Vector3(Math.sin(angle) * outer, Math.cos(angle) * outer, 0),
      ];
  ...
  const arrowMat = useMemo(() => new THREE.MeshBasicMaterial({ color: C_ACCENT, side: THREE.DoubleSide, transparent: true, opacity: 0.95, depthTest: false }), []);

  /* Cardinal labels — larger canvas for readability */
  const cardinalTex = useCallback((letter: string) => {
    const S = 128;                          // was 64, now 2× bigger
    const cv = document.createElement('canvas'); cv.width = cv.height = S;
    const ctx = cv.getContext('2d')!;
    ctx.clearRect(0, 0, S, S);
    ctx.fillStyle = letter === 'N' ? C_ACCENT : '#4a4845';
    ctx.font = `800 62px "Syne","DM Sans",system-ui,sans-serif`;   // was 28px
    ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
  ...
  const cardinals = useMemo(() => [
    ...
    letter,
    tex: cardinalTex(letter),
    pos: new THREE.Vector3(Math.sin(angle) * 1.18, Math.cos(angle) * 1.18, 0),
  })), [cardinalTex]);

  const labelGeo = useMemo(() => new THREE.PlaneGeometry(0.30, 0.30), []); // was 0.22

  /* Thick tick material — linewidth trick: render multiple parallel lines */
  const tickMat = useMemo(() => new THREE.LineBasicMaterial({
    color: C_ACCENT, transparent: true, opacity: 0.90, depthTest: false,
  }), []);
  ...
  return (
    <group position={[0, -0.58, 0]}>
      <group ref={ringRef}>
        ...
        {/* Ticks — render each tick 3 times offset slightly to fake thickness */}
        {tickLines.map((geo, ti) =>
          [-0.012, 0, 0.012].map((off, oi) => {
            const shifted = geo.clone();
            const arr = shifted.getAttribute('position') as THREE.BufferAttribute;
            // shift perpendicular to tick direction
            const angle = [0, Math.PI/2, Math.PI, -Math.PI/2][ti];
            const px = Math.cos(angle) * off;
            const py = -Math.sin(angle) * off;
            for (let i = 0; i < arr.count; i++) {
              arr.setX(i, arr.getX(i) + px);
              arr.setY(i, arr.getY(i) + py);
            }
            arr.needsUpdate = true;
            return <lineSegments key={`t${ti}-${oi}`} geometry={shifted} material={tickMat} renderOrder={2} />;
          })
        )}

        {/* North arrow */}
        <mesh geometry={arrowGeo} material={arrowMat} renderOrder={3} />
...
const ViewCube: React.FC = () => {
  ...
  const FACES: {label:string;pos:V3;rot:V3;dir:V3;up:V3;flipH?:boolean}[] = [
    ...
    {label:'BTM',   pos:[0,-P,0],  rot:[Math.PI/2,0,0],  dir:[0,-1,0], up:[0,0,1]},
  ];
    ...
    <>
      <ErrorBoundary>
        <Canvas shadows gl={{antialias:true,alpha:false,preserveDrawingBuffer:true,powerPreference:'high-performance',logarithmicDepthBuffer:true}} dpr={[1,2]} onContextMenu={e=>e.preventDefault()} onCreated={handleCreated}>
          ...
          {/*
            scale: 55 → 42  (smaller cube)
            margin right: 116 → stays (already shifted right)
            margin bottom: 110 → 120 (a bit more room for compass)
          */}
          <GizmoHelper alignment="bottom-right" margin={[116, 120]}>
            <group scale={42}>
              <ViewCube/>
            </group>
          </GizmoHelper>
